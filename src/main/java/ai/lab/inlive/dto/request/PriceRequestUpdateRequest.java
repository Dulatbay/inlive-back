...
public class PriceRequestUpdateRequest {
    ...
    @Schema(description = "Новый статус заявки (ACCEPTED, RAISED, DECREASED)", example = "ACCEPTED", required = true)
    private PriceRequestStatus status;

    @NotNull(message = "Price is required")
    @DecimalMin(value = "0.0", message = "Price must be non-negative")
    @Schema(description = "Новая цена (может быть изменена при RAISED или DECREASED)", example = "55000.0", required = true)
